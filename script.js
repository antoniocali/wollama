...
function filterAndRender(models, hw) {
  ...
  summaryEl.textContent = `${scored.length} model${
    scored.length === 1 ? "" : "s"
  } match your filters. Top results are biased towards your detected hardware.`;

  const topScore = scored[0].score;

  scored.forEach(({ model, score }) => {
    ...
    toolsChip.textContent = model.supports_tools ? "Tools / function calling" : "No tools";
    ...
    providerLine.innerHTML = `<span class="wm-small-label">Model</span><br>${model.model_name} · ${model.provider}`;
